...
between ::
  ...
  -> Parser a
  -> Parser a
between a b c = do { result <- a >> c; _ <- b; return result }
...
hex ::
  Parser Char
hex = do
  digits <- thisMany 4 (satisfy isHexDigit)
  case readHex digits of
    Empty -> failed
    Full ch -> return (chr ch)
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby p s = sepby1 p s ||| valueParser Nil
...
eof ::
  Parser ()
eof = P $ \inp -> if isEmpty inp
                  then Result inp ()
                  else ErrorResult $ ExpectedEof inp
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll fs = satisfy (\ch -> all ($ ch) fs)
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny fs = satisfy (\ch -> any ($ ch) fs)
